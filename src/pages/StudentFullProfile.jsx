...
function StudentSearch({ onSelect }) {
  ...
  const { data: nameResults = [], isLoading: searchingByName } =
    useGetStudentsQuery(nameSearch ? { search: nameSearch } : undefined, {
      skip: !nameSearch,
    });
  ...
  return (
    <div className="card space-y-3">
      ...
      <p className="text-sm text-gray-500">
        Kisi bhi student ka poora record — basic detail, academic record,
        fee/fine aur letters — ek hi page par dekhein aur update karein. Pehle
        student ko <b>Enrollment Number</b> se search karein.
      </p>
      ...
      {notFound && !verifying && enrollment && (
        <p className="text-sm text-red-600">
          Is enrollment number ka koi student database mein nahi mila — dobara
          check karein.
        </p>
      )}
      {exactStudent && !notFound && (
        <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
          <p className="text-sm text-green-700">
            ✓ Mil gaya: <b>{exactStudent.name}</b> (
            {exactStudent.enrollmentNumber})
          </p>
          <button
            className="btn-primary"
            onClick={() => onSelect(exactStudent.id)}
          >
            Open Full Record →
          </button>
        </div>
...
function BasicInfoSection({ student, canEdit }) {
  ...
  const handleToggleCategory = async () => {
    if (!canEdit) return;
    const next =
      student.studentCategory === "NewAdmission"
        ? "Continuing"
        : "NewAdmission";
    try {
      ...
      toast.success(
        next === "Continuing"
          ? "Moved to Study (Continuing) list"
          : "Moved back to New Admission list",
      );
    } catch (err) {
...
function AcademicRecordRow({ record, onUpdate, onDelete, canEdit }) {
  ...
  const handleSave = async () => {
    await onUpdate(record.id, {
      sessionStartYear: form.sessionStartYear
        ? Number(form.sessionStartYear)
        : undefined,
      sessionEndYear: form.sessionEndYear
        ? Number(form.sessionEndYear)
        : undefined,
      totalMarks: form.totalMarks !== "" ? Number(form.totalMarks) : undefined,
      obtainedMarks:
        form.obtainedMarks !== "" ? Number(form.obtainedMarks) : undefined,
    });
    setEditing(false);
  };
...
function AcademicRecordsSection({ studentId, canEdit }) {
  const { data: records = [], refetch } =
    useGetAcademicRecordsByStudentQuery(studentId);
    ...
    try {
      await createRecord({
        ...
        sessionEndYear: form.sessionEndYear
          ...
          : undefined,
        totalMarks:
          form.totalMarks !== "" ? Number(form.totalMarks) : undefined,
        obtainedMarks:
          form.obtainedMarks !== "" ? Number(form.obtainedMarks) : undefined,
      }).unwrap();
  ...
  return (
    <div className="card space-y-4">
      <h2 className="font-semibold">
        Academic Records (Matric / Inter / Degree)
      </h2>
      {canEdit && (
        <form onSubmit={handleAdd} className="flex flex-wrap gap-2 items-end">
          ...
          <div>
            <label className="text-xs text-gray-500">
              {form.level === "Degree" ? "Session Start Year" : "Session Start"}
            </label>
            <input
              type="number"
          ...
          {form.level === "Degree" && (
            <p className="text-xs text-gray-400 pb-2">
              End year auto-calculated from program duration.
            </p>
          )}
          <div>
            ...
            {records.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center text-gray-400 py-6">
                  Abhi koi academic record nahi hai
                </td>
              </tr>
            )}
            {records.map((r) => (
...
function LettersSection({ studentId, canEdit }) {
  const {
    data: letters = [],
    isLoading,
    refetch,
  } = useGetLettersByStudentQuery(studentId);
...
function FeeSection({ student, canEdit }) {
  ...
  const handleGenerate = async () => {
    if (!student?.semesterSystem) {
      toast.error(
        "Pehle Basic Info mein Program Duration (4-year / 5-year) set karein",
      );
      return;
    }
    try {
      ...
      toast.success(
        `${programSemesters} semesters generated (${student.semesterSystem} program)`,
      );
    } catch {
  ...
  return (
    <div className="card space-y-4">
      ...
      {semesters.length === 0 && canEdit && (
        <div className="flex flex-wrap items-end gap-3 border rounded-lg p-3">
          ...
          <button className="btn-primary" onClick={handleGenerate}>
            Generate Semesters{" "}
            {student?.semesterSystem
              ? `(${student.semesterSystem}, ${programSemesters} semesters)`
                    ...
                    return (
                      <tr key={f.id}>
                        ...
                        <EditableCell
                          ...
                          onSave={(v) => handleSaveField(f.id, "paidStatus", v)}
                        />
                        <EditableCell
                          disabled={!canEdit}
                        ...
                        {customFields.map((cf) => (
                          <EditableCell
                            ...
                            onSave={(v) =>
                              handleSaveField(f.id, "customValues", {
                                [cf.name]: v,
                              })
                            }
                          />
                          ...
                          <div className="flex flex-wrap gap-1">
                            {STATUS_TABS.map((t) => (
                              <button
                                ...
                                disabled={!canEdit}
                                onClick={() =>
                                  updateStatus({ id: f.id, [t.key]: !f[t.key] })
                                }
                                className={`text-xs px-2 py-1 rounded-full border ${f[t.key] ? "bg-primary-600 text-white border-primary-600" : "border-gray-200 text-gray-500"} ${!canEdit ? "cursor-default opacity-70" : ""}`}
                              >
                                {t.label}
                              </button>
                            ))}
      ...
      {semesters.length > 0 && canEdit && (
        <button
          className="btn-secondary"
          onClick={() => extendSemesters(studentId)}
        >
          Extend Semester Window (+{programYears} years / +{programSemesters}{" "}
          semesters)
        </button>
      )}
    </div>
...
export default function StudentFullProfile() {
  ...
  const canEdit = ["Manager", "Registrar"].includes(user?.role);
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <p className="text-gray-500 text-sm">
                ...
                <span
                  className={
                    liveStudent.studentCategory === "NewAdmission"
                      ? "text-amber-600"
                      : "text-green-600"
                  }
                >
              ...
              {!canEdit && (
                <p className="text-xs text-gray-400 mt-1">
                  View only — you don't have permission to edit this record.
                </p>
              )}
            </div>
            <button
